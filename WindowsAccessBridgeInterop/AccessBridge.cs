...
namespace WindowsAccessBridgeInterop
{
    ...
    public class AccessBridge : IDisposable
    {
        ...
        private static AccessBridgeLibrary LoadLibrary(string baseDirectory)
        {
            try
            {
                ...
                if (IntPtr.Size == 4)
                {
                    try
                    {
                        library = new AccessBridgeLibrary(System.IO.Path.Combine(baseDirectory, "WindowsAccessBridge-32.dll"));
                    }
                    catch
                    {
                        try
                        {
                            library = new AccessBridgeLibrary(System.IO.Path.Combine(baseDirectory, "WindowsAccessBridge.dll"))
                            {
                                IsLegacy = true
                            };
                        }
                ...
                {
                    library = new AccessBridgeLibrary(System.IO.Path.Combine(baseDirectory, "WindowsAccessBridge-64.dll"));
                }
                else
                {
